...
def _make_model(name, task, X_train, missing_values, seed, rf_trees, ff_kwargs):
    if name == "FastForest":
...
def _fit_score_auto(task, X_train, y_train, X_test, y_test, missing_values, ff_kwargs, autogrow):
...
def _fit_score(name, task, X_train, y_train, X_test, y_test, missing_values, rf_trees, ff_kwargs):
    "Fit, time, and score one model."
    if name in ("AutoForest","Autogrow"):
        return _fit_score_auto(task, X_train, y_train, X_test, y_test, missing_values, ff_kwargs, name=="Autogrow")
    result = {}
    start = time.perf_counter()
    model = _make_model(name, task, X_train, missing_values, 42, rf_trees, ff_kwargs)
    model.fit(X_train, y_train)
...
def _evaluate(send, name, task, X_train, y_train, X_test, y_test, missing_values, rf_trees, ff_kwargs):
...
def _evaluate_ashrae(send, name, data_home, max_rows, rf_trees, ff_kwargs):
    "Load, chronologically split, and evaluate ASHRAE inside the timed process."
    try:
        try: os.setsid()
        except (AttributeError, PermissionError): pass
        send.send(("ready", None))
        dataset_name,X,y,missing_values,task = load_data(Dataset.ashrae, data_home)
...
def _run_timed(name, task, X_train, y_train, X_test, y_test, missing_values, rf_trees, ff_kwargs, timeout):
    "Evaluate a model in a fresh process, returning `None` on timeout."
    context = mp.get_context("spawn")
    receive,send = context.Pipe(False)
    args = (send, name, task, X_train, y_train, X_test, y_test, missing_values, rf_trees, ff_kwargs)
...
def _run_ashrae_timed(name, data_home, max_rows, rf_trees, ff_kwargs, timeout):
    "Load and evaluate ASHRAE in a fresh process, returning `None` on timeout."
    context = mp.get_context("spawn")
    receive,send = context.Pipe(False)
    args = (send, name, data_home, max_rows, rf_trees, ff_kwargs)
...
def main(
    ...
    ff_kwargs = dict(n_trees=ff_trees, bootstrap_fraction=bootstrap_fraction, replacement=_replacement(replacement),
        random_splitter=random_splitter, date_columns=None if dates else {})
    ...
    for name in models:
        ...
        if diagnostics and name == "FastForest":
            for bundle,members in result["bundles"]: print(f"{'bundle':<15} {bundle}: {', '.join(members)}")
