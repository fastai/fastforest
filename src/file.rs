...
use crate::{
    ClassifierForest, Config, Encoder, Forest, ForestError, MaxFeatures, ModelMetadata,
...
pub struct FileFitOptions {
    ...
    pub frequent_value_fraction: f32,
...
impl FileFitOptions {
    fn resolved_replacement(&self, rows: usize) -> bool {
        self.replacement.unwrap_or(rows < match self.task {
            Task::Regression => 10_000,
            Task::Classification => 40_000,
        })
    }
}
...
fn headers(path: &Path) -> Result<Vec<String>, ForestError> {
    ...
    let names: Vec<_> = reader
        .headers()
        .map_err(|error| file_error("could not read CSV header", error))?
        .iter()
        .map(str::to_owned)
        .collect();
...
fn predictor_layout(
...
) -> Result<(Vec<usize>, Vec<String>, ModelMetadata, Vec<(String, Vec<usize>)>, Vec<(usize, String, u8, String)>), ForestError> {
    ...
    let positions: HashMap<_, _> = predictor_names
        .iter()
        .enumerate()
    ...
    for (name, marker) in &options.missing_values {
        let index = positions
            .get(name.as_str())
            .copied()
            .ok_or_else(|| ForestError::new(format!("unknown missing-value column {name:?}")))?;
        marker.validate()?;
            ...
            let indices: Result<Vec<_>, _> = columns
                ...
                .map(|name| {
                    positions
                        .get(name.as_str())
                        .copied()
                        .ok_or_else(|| ForestError::new(format!("unknown one-hot column {name:?}")))
                })
                .collect();
    ...
    for (name, format) in &options.date_columns {
        let index = positions
            .get(name.as_str())
            .copied()
            .ok_or_else(|| ForestError::new(format!("unknown date column {name:?}")))?;
        let prefix = name
            .strip_suffix("Date")
            .or_else(|| name.strip_suffix("date"))
            .unwrap_or(name);
        for (part, suffix) in PARTS.iter().enumerate() {
            date_parts.push((index, format.clone(), part as u8, format!("{prefix}{suffix}")));
        }
        dates.push((index, format.clone()));
    }
    let metadata = ModelMetadata {
        markers,
        one_hot_groups: groups.clone(),
        date_columns: dates,
        parameters: Vec::new(),
    };
    Ok((sources, predictor_names, metadata, groups, date_parts))
}

fn target_sample_and_rows(
    path: &Path,
    target: usize,
    seed: Option<u64>,
) -> Result<(usize, Vec<String>), ForestError> {
    let mut reader = csv::Reader::from_path(path).map_err(|error| file_error("could not open CSV", error))?;
    ...
    for record in reader.records() {
        ...
        let value = record
            .get(target)
            .ok_or_else(|| ForestError::new("CSV row has the wrong number of columns"))?;
        if rows < 1_000 {
...
pub fn fit_csv(path: impl AsRef<Path>, options: &FileFitOptions) -> Result<SavedModel, ForestError> {
    ...
    let estimated_outputs = if options.task == Task::Classification {
        target_sample.iter().collect::<BTreeSet<_>>().len().saturating_sub(1).max(1)
    } else {
        1
    };
    let replacement = options.resolved_replacement(n_rows);
    ...
    let (sources, predictor_names, metadata, groups, date_parts) =
        predictor_layout(&names, target, options)?;
    let (batch, targets) = selected_csv_rows(path, &selected, &sources, target, &predictor_names)?;
    let (encoder, x) = Encoder::fit_arrow(
        &batch,
        &metadata.markers,
        options.max_dummy_cardinality,
        options.frequent_value_fraction,
        options.allow_new_missing,
        groups,
        date_parts,
    )?;
    let mut config = Config {
        n_trees: estimated.n_trees,
        ...
        tree_cutoff_samples: options.tree_cutoff_samples,
        min_local_gain: options.min_local_gain,
        min_global_gain: options.min_global_gain,
    ...
    match options.task {
        Task::Regression => {
            let y: Result<Vec<_>, _> = targets
                ...
                .map(|value| {
                    value
                        ...
                        .ok_or_else(|| ForestError::new(format!("invalid regression target {value:?}")))
                })
                .collect();
            let actual = plan_fit(
                n_rows,
            ...
            config.n_trees = actual.n_trees;
            config.sample_rows = Some(actual.rows_per_tree.min(x.nrows()));
        ...
        Task::Classification => {
            ...
            let classes: Vec<_> = targets.iter().cloned().collect::<BTreeSet<_>>().into_iter().collect();
            if classes.len() < 2 {
                return Err(ForestError::new("classification requires at least two classes"));
            }
            let lookup: HashMap<_, _> = classes
                .iter()
                .enumerate()
                .map(|(index, value)| (value.as_str(), index as u32))
                .collect();
            let y = Array1::from_iter(targets.iter().map(|value| lookup[value.as_str()]));
            ...
            let classes = classes
                .into_iter()
                .map(|value| SavedValue { kind: 5, value })
                .collect();
...
fn prediction_columns(
    ...
    let source_names: Vec<_> = reader
        ...
        .map(str::to_owned)
        .collect();
    let positions: HashMap<_, _> = source_names
        .iter()
        .enumerate()
        .map(|(index, name)| (name.as_str(), index))
        .collect();
    let sources: Result<Vec<_>, _> = names
        ...
        .map(|name| {
            positions
                .get(name.as_str())
                .copied()
                .ok_or_else(|| ForestError::new(format!("prediction column {name:?} is missing")))
        })
        .collect();
    let sources = sources?;
        ...
        SavedModel::Classification { forest, classes, .. } => {
            ...
            prediction_columns(input, encoder.input_names(), batch_size, |batch| {
                ...
                for class in forest.predict(x.view())? {
                    writer.write_record([class_text(&classes[class as usize])])
                        .map_err(|error| file_error("could not write CSV", error))?;
                }
                Ok(())
            })?;
...
fn arrow_saved_value(array: &dyn Array, row: usize) -> Result<Option<SavedValue>, ForestError> {
    ...
    let (kind, value) = match array.data_type() {
        ...
        | DataType::Date32 | DataType::Date64 | DataType::Timestamp(_, _) | DataType::Time32(_)
        | DataType::Time64(_) => (5, array_value_to_string(array, row)
            .map_err(|error| file_error("could not read Arrow target", error))?),
        data_type => return Err(ForestError::new(format!("unsupported Arrow target type {data_type}"))),
    };
    Ok(Some(SavedValue { kind, value }))
}

fn concat_batches(batches: &[RecordBatch], schema: Arc<Schema>) -> Result<RecordBatch, ForestError> {
    let columns: Result<Vec<_>, _> = (0..schema.fields().len()).map(|column| {
...
pub fn fit_arrow(path: impl AsRef<Path>, options: &FileFitOptions) -> Result<SavedModel, ForestError> {
    ...
    let replacement = options.resolved_replacement(rows);
    let estimated = plan_fit(
        rows,
        options.n_trees,
        options.bootstrap_fraction,
    ...
    selected.sort_unstable();
    let (sources, _, metadata, groups, date_parts) = predictor_layout(&names, target, options)?;
    ...
    for batch in &mut reader {
        ...
        if next > start {
            ...
            let columns: Result<Vec<_>, _> = batch.columns().iter().map(|array| take(array.as_ref(), &indices, None)
                .map_err(|error| file_error("could not select sampled Arrow rows", error))).collect();
            batches.push(RecordBatch::try_new(batch.schema(), columns?)
                .map_err(|error| file_error("could not build sampled Arrow batch", error))?);
        }
        global_row = end;
    }
    ...
    let targets: Result<Vec<_>, _> = (0..selected_batch.num_rows())
        .map(|row| arrow_saved_value(selected_batch.column(target).as_ref(), row)).collect();
    let targets = targets?;
        ...
        Task::Regression => {
            let y: Result<Vec<_>, _> = targets
                ...
                .map(|value| {
                    value
                        .as_ref()
                        .ok_or_else(|| ForestError::new("regression targets cannot be missing"))?
                        .value
                        .parse::<f32>()
                        .map_err(|_| ForestError::new("invalid regression target"))
                })
                .collect();
            let actual = plan_fit(rows, options.n_trees, options.bootstrap_fraction, options.bootstrap_max,
                replacement, options.oob, 1)?;
            config.n_trees = actual.n_trees;
            config.sample_rows = Some(actual.rows_per_tree.min(x.nrows()));
            let forest = Forest::fit(x.view(), Array1::from_vec(y?).view(), encoder.cutoff_values(),
                encoder.cutoff_offsets(), Some(encoder.feature_group_ids()), Some(encoder.frequent_parents()), &config)?;
            Ok(SavedModel::regression(encoder, forest, metadata))
        }
        Task::Classification => {
            ...
            let dimensions = classes.len().saturating_sub(1).max(1);
            let actual = plan_fit(rows, options.n_trees, options.bootstrap_fraction, options.bootstrap_max,
                replacement, options.oob, dimensions)?;
            config.n_trees = actual.n_trees;
            config.sample_rows = Some(actual.rows_per_tree.min(x.nrows()));
            let forest = ClassifierForest::fit(x.view(), y.view(), classes.len(), encoder.cutoff_values(),
                encoder.cutoff_offsets(), Some(encoder.feature_group_ids()), Some(encoder.frequent_parents()), &config)?;
            Ok(SavedModel::classification(encoder, forest, metadata, classes))
        }
    }
...
pub fn predict_arrow(
    ...
    let sources: Result<Vec<_>, _> = encoder.input_names().iter().map(|name| positions.get(name.as_str()).copied()
        .ok_or_else(|| ForestError::new(format!("prediction column {name:?} is missing")))).collect();
    let sources = sources?;
    ...
    for batch in &mut reader {
        ...
        for offset in (0..batch.num_rows()).step_by(batch_size) {
            ...
            let arrays: Vec<ArrayRef> = match model {
                SavedModel::Regression { forest, .. } => vec![Arc::new(Float32Array::from(forest.predict(x.view())?))],
                SavedModel::Classification { forest, classes, .. } if proba => {
                    let probabilities = forest.predict_proba(x.view())?;
                    (0..classes.len()).map(|class| Arc::new(Float32Array::from(
                        probabilities.chunks_exact(classes.len()).map(|row| row[class]).collect::<Vec<_>>())) as ArrayRef).collect()
                }
...
pub fn convert_csv_to_arrow(
        ...
        for (column, value) in record.iter().enumerate() {
            ...
            if value.parse::<f64>().is_ok_and(f64::is_finite) {
                ...
                return Err(ForestError::new(format!(
                    "column {:?} has nonnumeric value {value:?} at row {row}", names[column]
                )));
            }
        }
    }
    let schema = Arc::new(Schema::new(names.iter().zip(&kinds).map(|(name, kind)| Field::new(name,
        if matches!(kind, ConvertedKind::Integer) { DataType::Int64 } else { DataType::Float64 }, true)).collect::<Vec<_>>()));
    ...
    let write_rows = |rows: &mut Vec<Vec<Option<String>>>, writer: &mut FileWriter<File>| -> Result<(), ForestError> {
        ...
        let arrays: Result<Vec<ArrayRef>, ForestError> = rows.iter().zip(&kinds).map(|(values, kind)| match kind {
            ConvertedKind::Integer => Ok(Arc::new(Int64Array::from(values.iter().map(|value| value.as_ref().map(|value| value.parse::<i64>().unwrap())).collect::<Vec<_>>())) as ArrayRef),
            ConvertedKind::Float => Ok(Arc::new(Float64Array::from(values.iter().map(|value| value.as_ref().map(|value| value.parse::<f64>().unwrap())).collect::<Vec<_>>())) as ArrayRef),
        }).collect();
...
mod tests {
    ...
    #[test]
    fn csv_fit_save_and_streaming_prediction_story() {
        ...
        for row in 0..240 {
            ...
            let target = row as f32 * 0.5 + (row % 3) as f32 * 4.0;
            let label = if row % 3 == 0 { "low" } else if row % 3 == 1 { "middle" } else { "high" };
        ...
        let mixed_arrow = directory.path().join("mixed.arrow");
        let mixed_schema = Arc::new(Schema::new(vec![
            Field::new("number", DataType::Int64, false), Field::new("color", DataType::Utf8, false),
            Field::new("target", DataType::Float32, false), Field::new("label", DataType::Utf8, false),
        ]));
        let mixed_batch = RecordBatch::try_new(mixed_schema.clone(), vec![
            Arc::new(Int64Array::from_iter_values(0..240)),
            Arc::new(StringArray::from((0..240).map(|row| ["red", "green", "blue"][row%3]).collect::<Vec<_>>())),
            Arc::new(Float32Array::from((0..240).map(|row| row as f32*0.5+(row%3) as f32*4.0).collect::<Vec<_>>())),
            Arc::new(StringArray::from((0..240).map(|row| ["low", "middle", "high"][row%3]).collect::<Vec<_>>())),
        ]).unwrap();
        let mut mixed_writer = arrow_output_writer(&mixed_arrow, mixed_schema).unwrap();
        mixed_writer.write(&mixed_batch).unwrap();
        mixed_writer.finish().unwrap();
        let mixed_model = fit_arrow(&mixed_arrow, &FileFitOptions { task:Task::Classification, target:"label".into(),
            n_trees:Some(12), seed:Some(42), ..FileFitOptions::default() }).unwrap();
        let mixed_predictions = directory.path().join("mixed-predictions.arrow");
        predict_arrow(&mixed_model, &mixed_arrow, &mixed_predictions, 19, true).unwrap();
        assert_eq!(arrow_reader(&mixed_predictions).unwrap().map(|batch| batch.unwrap().num_rows()).sum::<usize>(), 240);
        ...
        let arrow = directory.path().join("numeric.arrow");
        convert_csv_to_arrow(&numeric_csv, &arrow, 23).unwrap();
        let arrow_model = fit_file(
            &arrow,
            &FileFitOptions {
                target: "target".to_owned(),
                n_trees: Some(8),
        ...
        .unwrap();
        let arrow_predictions = directory.path().join("predictions.arrow");
        predict_file(&arrow_model, &arrow, &arrow_predictions, 17, false).unwrap();
        let predicted_rows: usize = arrow_reader(&arrow_predictions)
            .unwrap()
            .map(|batch| batch.unwrap().num_rows())
            .sum();
        assert_eq!(predicted_rows, 211);
    }
}
