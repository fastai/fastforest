...
impl SavedValue {
    pub fn validate(&self) -> Result<(), ForestError> {
        ...
        match self.kind {
            2 if self.value != "0" && self.value != "1" => {
                Err(ForestError::new("saved model contains an invalid boolean"))
            }
            3 => self
                .value
                .parse::<i64>()
                .map(|_| ())
                .map_err(|_| ForestError::new("saved model contains an invalid integer")),
            4 => self
                .value
                .parse::<f64>()
                .map(|_| ())
                .map_err(|_| ForestError::new("saved model contains an invalid float")),
            _ => Ok(()),
        }
    }
}
...
pub enum SavedModel {
    ...
    Classification {
        encoder: Encoder,
        forest: ClassifierForest,
        metadata: ModelMetadata,
        classes: Vec<SavedValue>,
    },
}
...
impl SavedModel {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, ForestError> {
        ...
        let (envelope, used): (Envelope, usize) = bincode::serde::decode_from_slice(
            &bytes[MAGIC.len()..],
            bincode::config::standard(),
        )
    ...
    pub fn load(path: impl AsRef<Path>) -> Result<Self, ForestError> {
        ...
        let mut file = File::open(path)
            .map_err(|error| ForestError::new(format!("could not open {:?}: {error}", path)))?;
        let mut bytes = Vec::new();
        file.read_to_end(&mut bytes)
            .map_err(|error| ForestError::new(format!("could not read {:?}: {error}", path)))?;
    ...
    pub fn save(&self, path: impl AsRef<Path>) -> Result<(), ForestError> {
        ...
        let mut temporary = NamedTempFile::new_in(parent)
            .map_err(|error| ForestError::new(format!("could not create model file: {error}")))?;
        temporary
            .write_all(&bytes)
        ...
        temporary
            .persist(path)
            .map_err(|error| ForestError::new(format!("could not save {:?}: {}", path, error.error)))?;
        Ok(())
    }
