...
pub(crate) const FEATURE_MASK: u32 = EQUALITY_BIT - 1;
...
pub(crate) fn leaf_index<C: Copy + PartialEq, P>(
    nodes: &[TreeNode<C, P>], value: impl Fn(usize) -> C, ordered_right: impl Fn(C, C) -> bool,
) -> usize {
    let mut node_idx = 0;
    loop {
        ...
        let go_right = usize::from(if node.equality() { observed != node.cut_val } else { ordered_right(observed, node.cut_val) });
...
pub(crate) fn split_children<C: Copy, P: Default>(
    nodes: &mut Vec<TreeNode<C, P>>, node_idx: usize, cut_col: usize, cut_val: C, equality: bool,
) -> (usize, usize)
where
    C: Default,
{
    ...
    nodes[node_idx].cut_col = u32::try_from(cut_col).expect("matrix has too many columns") | if equality { EQUALITY_BIT } else { 0 };
    nodes[node_idx].cut_val = cut_val;
    (left, left + 1)
}

pub(crate) fn grow_tree<P: Default>(
    x: ArrayView2<'_, u32>, rows: &mut [u32], nodes: &mut Vec<TreeNode<u32, P>>, importance: &mut [f32],
    ...
    while let Some((node_idx, start, n_rows)) = work.pop() {
        ...
        let left_n = partition(x, rows, start, n_rows, branch.cut_col, branch.cut_val, branch.equality);
        debug_assert!(left_n > 0 && left_n < n_rows);
        let (left_idx, right_idx) = split_children(nodes, node_idx, branch.cut_col, branch.cut_val, branch.equality);
